...
passwordInput.addEventListener('input', () => {
  ...
  const score = 5 - Object.values(errors).filter(Boolean).length;
...
// ✅ async aqui é obrigatório para o await funcionar
document.getElementById('registerForm').addEventListener('submit', async function (e) {
  ...
  if (!isValidCPF(cpf)) { setError('cpf', 'cpfError', true); valid = false; }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setError('email', 'emailError', true); valid = false; }
  if (username.length < 3) { setError('username', 'usernameError', true); valid = false; }

  const pwdErrors = validatePasswordDetailed(password);
  let hasPwdError = false;
  for (let key in pwdErrors) {
    if (pwdErrors[key]) { setErrorVisibility(`passwordError${capitalize(key)}`, true); hasPwdError = true; }
    else { setErrorVisibility(`passwordError${capitalize(key)}`, false); }
  }
  ...
  // ✅ Só chama a API se tudo for válido, sem redirect antes
  if (!valid) return;
  ...
  try {
    ...
    // ✅ Só redireciona após cadastro bem-sucedido
